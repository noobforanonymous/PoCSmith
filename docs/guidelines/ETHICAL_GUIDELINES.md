# Ethical Guidelines and Legal Disclaimer

## IMPORTANT LEGAL NOTICE

**PoCSmith is designed for:**
- Security research
- Penetration testing (authorized)
- CTF competitions
- Educational purposes
- Vulnerability disclosure programs

**PoCSmith is NOT designed for:**
- Unauthorized access to systems
- Malicious hacking
- Criminal activities
- Harm to individuals or organizations

## Responsible Use

### Before Using PoCSmith

1. **Obtain Written Permission**
   - Never test systems you do not own without explicit authorization
   - Get written permission from system owners
   - Document scope and boundaries of testing

2. **Know the Law**
   - Computer Fraud and Abuse Act (CFAA) - USA
   - Computer Misuse Act - UK
   - Local cybersecurity laws in your jurisdiction
   - Unauthorized access is illegal in most countries

3. **Follow Responsible Disclosure**
   - Report vulnerabilities to vendors privately
   - Allow reasonable time for patches
   - Coordinate public disclosure responsibly

### During Security Testing

1. **Stay Within Scope**
   - Only test systems explicitly authorized
   - Do not exceed agreed-upon testing boundaries
   - Stop immediately if you encounter unexpected access

2. **Minimize Impact**
   - Avoid denial of service attacks
   - Do not modify or delete production data
   - Test during agreed-upon time windows

3. **Document Everything**
   - Keep detailed logs of all testing activities
   - Document vulnerabilities discovered
   - Note exploitation steps for reporting

### After Discovery

1. **Report Responsibly**
   - Contact vendor security teams
   - Provide clear reproduction steps
   - Suggest remediation if possible

2. **Protect Information**
   - Do not publicly disclose before patches available
   - Do not share exploits with unauthorized parties
   - Maintain confidentiality agreements

## Legal Disclaimer

I, Regaan, the creator of PoCSmith, provide this tool AS-IS with NO WARRANTY.

**By using PoCSmith, you agree:**

1. You are solely responsible for how you use this tool
2. You will comply with all applicable laws in your jurisdiction
3. You will only use this tool for authorized testing with proper permission
4. You understand the legal risks associated with security testing
5. The creator bears no responsibility for misuse

## Consequences of Misuse

**Criminal Penalties:**
- Federal prosecution under CFAA (USA)
- Prison sentences up to 20 years
- Fines up to $250,000
- Permanent criminal record

**Civil Liability:**
- Lawsuits from affected parties
- Damages and restitution
- Legal fees

**Professional Consequences:**
- Loss of security certifications
- Termination of employment
- Industry blacklisting

## Ethical Hacking Best Practices

1. **Always get permission in writing**
2. **Report vulnerabilities responsibly**
3. **Follow coordinated disclosure timelines**
4. **Never profit from unauthorized exploitation**
5. **Respect user privacy and data**
6. **Use exploits only for defensive purposes**

---

By using PoCSmith, you acknowledge that you have read, understood, and agree to follow these ethical guidelines and accept all legal responsibilities.

Regaan (RoTHackers)
